...
!
! Modifications:
!	2011-06-27, AJD: Initial version
!	2025-08-20,  JB: Added C2R, R2C, Z2D, D2Z 2d execution interfaces
...
interface cufftExec
	...
	!2D execution
    ! complex to complex 2D execution forward and inverse
	subroutine cufftExecC2C_2d(plan, idata, odata, direction) bind(c,name='cufftExecC2C') 
	use iso_c_binding
	!use m_precision ! removed, the precision is single explicitly here
	integer(c_int),value:: direction
	integer(c_int),value:: plan
	complex(4),device,dimension(:,:) :: idata
	complex(4),device,dimension(:,:) :: odata
    end subroutine cufftExecC2C_2d
    ! real to complex 2D execution (forward implied)
    subroutine cufftExecR2C_2d(plan, idata, odata) bind(c,name='cufftExecR2C') 
	use iso_c_binding
	integer(c_int),value:: plan
	real(4),device,dimension(:,:) :: idata
	complex(4),device,dimension(:,:) :: odata
    end subroutine cufftExecR2C_2d
    ! complex to real 2D execution (inverse implied)
    subroutine cufftExecC2R_2d(plan, idata, odata) bind(c,name='cufftExecC2R') 
	use iso_c_binding
	integer(c_int),value:: plan
	complex(4),device,dimension(:,:) :: idata
	real(4),device,dimension(:,:) :: odata
    end subroutine cufftExecC2R_2d
	...
	!2D execution
    ! double complex to double complex 2D execution forward and inverse
	subroutine cufftExecZ2Z_2d(plan, idata, odata, direction) bind(c,name='cufftExecZ2Z') 
	use iso_c_binding
    !use m_precision ! removed, the precision is double explicitly here
	integer(c_int),value:: direction
	integer(c_int),value:: plan
	complex(8),device,dimension(:,:) :: idata
	complex(8),device,dimension(:,:) :: odata
    end subroutine cufftExecZ2Z_2d
    ! double to double complex 2D execution (forward implied)
    subroutine cufftExecD2Z_2d(plan, idata, odata) bind(c,name='cufftExecD2Z') 
	use iso_c_binding
	integer(c_int),value:: plan
	real(8),device,dimension(:,:) :: idata
	complex(8),device,dimension(:,:) :: odata
    end subroutine cufftExecD2Z_2d
    ! double complex to double 2D execution (inverse implied)
    subroutine cufftExecZ2D_2d(plan, idata, odata) bind(c,name='cufftExecZ2D') 
	use iso_c_binding
	integer(c_int),value:: plan
	complex(8),device,dimension(:,:) :: idata
