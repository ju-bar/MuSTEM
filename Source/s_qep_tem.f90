...
psi_initial_d = psi_initial
        ...
        do i_cell = 1,maxval(ncells)
	        do i_slice = 1, n_slices
				...
				if (plasmonmc) then ! use plasmon code
					pl_tslc = prop_distance(i_slice) ! store current slice thickness
					pl_grid_sqx = 1.0_fp_kind / a0_slice(1,i_slice) / ak1 ! store reciprocal grid sampling along x in rad/pixel
					pl_grid_sqy = 1.0_fp_kind / a0_slice(2,i_slice) / ak1 ! store reciprocal grid sampling along y in rad/pixel
    ...
    do i_qep_pass = 1, n_qep_passes 
        ...
        do i_cell = 1,maxval(ncells)
			...
			!If this thickness is an output thickness then accumulate relevant TEM images and diffraction patterns
			if (any(i_cell==ncells)) then
