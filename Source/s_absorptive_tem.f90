...
subroutine absorptive_tem
	...
	complex(fp_kind),dimension(:,:),allocatable :: psi,psi_initial,psi_out,prop ! inserted by JB 2202-06-10 (prevents large stack allocation)
    !complex(fp_kind) :: lens_ctf(nopiy,nopix,imaging_ndf)
	complex(fp_kind), allocatable :: lens_ctf(:,:,:) ! inserted by JB 2202-06-10 (prevents large stack allocation)
    !complex(fp_kind),dimension(nopiy,nopix,n_slices):: projected_potential, transf_absorptive
	complex(fp_kind),allocatable,dimension(:,:,:) :: projected_potential, transf_absorptive ! inserted by JB 2202-06-10 (prevents large stack allocation)
    
    !output
    !real(fp_kind),dimension(nopiy,nopix) :: cbed,image,tem_image,temp_image
	real(fp_kind),dimension(:,:),allocatable :: cbed,image,tem_image,temp_image ! inserted by JB 2202-06-10 (prevents large stack allocation)
	...
	complex(fp_kind),device,allocatable :: psi_d(:,:) ! inserted by JB 2202-06-10 (prevents large stack allocation)
	...
	allocate(psi_d(nopiy,nopix)) ! inserted by JB 2202-06-10 (prevents large stack allocation)
#else
	on_the_fly=.false.
#endif
	! local big array allocations on heap ! inserted by JB 2202-06-10 (prevents large stack allocation)
	allocate(projected_potential(nopiy,nopix,n_slices)) ! inserted by JB 2202-06-10 (prevents large stack allocation)
	allocate(transf_absorptive(nopiy,nopix,n_slices)) ! inserted by JB 2202-06-10 (prevents large stack allocation)
	allocate(lens_ctf(nopiy,nopix,imaging_ndf)) ! inserted by JB 2202-06-10 (prevents large stack allocation)
	allocate(psi(nopiy,nopix),psi_initial(nopiy,nopix),psi_out(nopiy,nopix),prop(nopiy,nopix)) ! inserted by JB 2202-06-10 (prevents large stack allocation)
	allocate(cbed(nopiy,nopix),image(nopiy,nopix),tem_image(nopiy,nopix),temp_image(nopiy,nopix)) ! inserted by JB 2202-06-10 (prevents large stack allocation)
	!
    manyz = nz>1
...
901     format(1h+,1x, 'Cell: ', i5, ' tilt:',i5,'/',i5,' Intensity: ', f12.6)	
	...
	deallocate(transf_absorptive, projected_potential, lens_ctf) ! inserted by JB 2202-06-10 (prevents large stack allocation)
    deallocate(psi, psi_initial, psi_out, prop) ! inserted by JB 2202-06-10 (prevents large stack allocation)
	deallocate(cbed, image, tem_image, temp_image) ! inserted by JB 2202-06-10 (prevents large stack allocation)
